...
const emailCheck = async (email) => {
    ...
    if (Number(Object.values(userEmail[0])[0])) throw new BaseError("EMAIL_DUPLICATE", 409)

    if (!Number(Object.values(userEmail[0])[0])) throw new BaseError("EMAIL_NOT_EXISTS", 200)
};

const signUp = async (email, lastName, firstName, profileImgUrl, password, birthday) => {
    validateEmail(email);
    validatePassword(password);

    const userEmail = await userDao.userEmailCheck(email);

    if (Number(Object.values(userEmail[0])[0])) throw new BaseError("DUPLICATED_EMAIL_INFO", 400)
...
const signIn = async (email, password) => {
    ...
    if (!Number(Object.values(user)[0])) throw new BaseError("INVALID_INFORMATION", 400)
